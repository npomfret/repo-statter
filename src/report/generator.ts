...
function injectDataIntoTemplate(template: string, chartData: ChartData, commits: CommitData[]): string {
  ...
  const commitActivityChartClass = `
    class CommitActivityChart {
      constructor(containerId) {
        this.containerId = containerId;
        this.chart = null;
      }
      render(linearSeries, timeSeries, xAxis) {
        assert(linearSeries !== undefined, 'Linear series data is required');
        assert(timeSeries !== undefined, 'Time series data is required');
        assert(Array.isArray(linearSeries), 'Linear series must be an array');
        assert(Array.isArray(timeSeries), 'Time series must be an array');
        ...
        const options = {
          ...
          xaxis: { 
            ...
            labels: { 
              style: { colors: isDark ? '#f0f6fc' : '#24292f' },
              formatter: xAxis === 'commit' ? function(val) { return Math.floor(val).toString(); } : undefined
            }
          },
          yaxis: { 
            title: { 
  ...
  const codeChurnChartClass = `
    class CodeChurnChart {
      constructor(containerId) {
        this.containerId = containerId;
        this.chart = null;
      }
      render(linearSeries, timeSeries, xAxis, commits) {
        ...
        // Build chart data based on x-axis selection
        const buildTimeSeriesData = (data, xAxis, yValueExtractor) => {
          if (xAxis === 'date') {
        ...
        const createCommitTooltip = (xAxis, linearSeries, commits, contentBuilder) => {
  ...
  const repositorySizeChartClass = `
    class RepositorySizeChart {
      constructor(containerId) {
        this.containerId = containerId;
        this.chart = null;
      }
      render(linearSeries, timeSeries, xAxis, commits) {
        ...
        const createCommitTooltip = (xAxis, filteredLinearSeries, commits, customContent) => {
  ...
  const chartScript = `
    <script>
      ...
      function renderCommitActivityChart() {
        ...
        commitActivityChart = new CommitActivityChart('commitActivityChart');
        commitActivityChart.render(filteredLinearSeries, filteredTimeSeries, xAxis);
      }
      ...
      function renderCodeChurnChart() {
        ...
        codeChurnChart = new CodeChurnChart('codeChurnChart');
        codeChurnChart.render(filteredLinearSeries, filteredTimeSeries, xAxis, commits);
      }
      ...
      function renderRepositorySizeChart() {
        ...
        repositorySizeChart = new RepositorySizeChart('repositorySizeChart');
        repositorySizeChart.render(filteredLinearSeries, filteredTimeSeries, xAxis, commits);
      }
      ...
      document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('themeToggle').addEventListener('change', function(e) {
          ...
          requestAnimationFrame(() => {
            ...
            // For the charts with instances, we must destroy them before re-rendering.
            if (commitActivityChart) {
              commitActivityChart.destroy();
            }
            if (linesOfCodeChart) {
